...
#@inline Base.length(A::AbstractMesh{T,N}) where {T<:Int,N} = A.multiplied_dims[N]
...
struct StencilDirections{T<:Int}
    up::T
    down::T
    left::T
    right::T
...
struct Stencil{T,N} <: AbstractArray{Int,N}
    center::NTuple{N,Int}
    dir::Vector{StencilDirections}
