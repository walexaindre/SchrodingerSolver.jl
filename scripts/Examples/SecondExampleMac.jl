...
const outputfolder = pwd() * "/scripts/Examples/Results/P2/"
...
function test(τv, hv, spord, tord)
    ...
    for index in 0:(tsteps - 1)
        ...
        if index in plot_iter
            ...
            grabstate(Mem,Mesh,1,output_name(nextind(),2))
            grabstate(Mem,Mesh,2,output_name(nextind(),2))
            grabstate(Mem,Mesh,3,output_name(nextind(),2))
        end
    ...
    save(output_name(nextind(),2) * "solviter.png", px; px_per_unit = 3)
    px = solvertime(Stats, u"ms")
    save(output_name(nextind(),2) * "solvtime.png", px; px_per_unit = 3)
    px = executiontime(Stats, u"ms")
    save(output_name(nextind(),2) * "exectime.png", px; px_per_unit = 3)
    px = systemenergy(Stats)
    save(output_name(nextind(),2) * "sysenergy.png", px; px_per_unit = 3)
    px = componentpower(Stats, 1)
    save(output_name(nextind(),2) * "syspowerc1.png", px; px_per_unit = 3)
    px = componentpower(Stats, 2)
    save(output_name(nextind(),2) * "syspowerc2.png", px; px_per_unit = 3)
    px = componentpower(Stats, 3)
    save(output_name(nextind(),2) * "syspowerc3.png", px; px_per_unit = 3)

    store(Stats, output_name(nextind(),2) * "stats.json")

    towrite = pde_parameters(2, τv, hv, spord, tord)
    (f -> open(f, "w") do io
         JSON.print(io, towrite)
     end)(output_name(nextind(),2) * "meta.json")
    stateidx += 1
end
